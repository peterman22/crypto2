...
import { adminApproveDeposit, adminGetDeposits, ssrAdminCheckTokenValidity } from "services";
...
const DepositsIndexPage = (props: IProps) => {
    ...
    const [filter, setFilter] = React.useState<'all' | 'pending' | 'active'>('all');
    ...
    const filterBy = () => {
        switch (filter) {
            ...
            case 'active':
                return deposits.filter((d: any) => d.status == 'active');
    ...
    const markAsActive = async (d: any) => {
                            ...
                            <div className="flex flex-row justify-between items-center w-full">
                                ...
                                <Dropdown>
                                    ...
                                    <Dropdown.Menu className={`text-gray-800 bg-white border border-gray-200 flex flex-col rounded-md p-2 text-xs`}>
                                        ...
                                        <Dropdown.Item onClick={() => setFilter('active')} className={`text-gray-800 border-b border-b-gray-100 pt-2`}>
                                            Active
                                        </Dropdown.Item>
                                    </Dropdown.Menu>
                                </Dropdown>
                                                        ...
                                                        <Dropdown.Menu className={`text-gray-800 bg-gray-200 border border-gray-300 font-semibold flex flex-col rounded-md p-2 text-xxs`}>
                                                            {
                                                                d.status == 'pending'
                                                                    ?
                                                                    <Dropdown.Item onClick={() => markAsActive(d)} className={`text-red-600 pt-1`}>
                                                                        <span className="">
                                                                            mark as active?
                                                                        </span>
                                                                    </Dropdown.Item>
