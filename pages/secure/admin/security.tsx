...
import { adminChangePassword, ssrAdminCheckTokenValidity } from "services";
...
const AdminSecurityIndexPage = (props: IProps) => {
    ...
    const changePassword = async () => {
        ...
        const r = await adminChangePassword(old, n);
    ...
    return (
        <AdminDashboardLayout overflow={true} user={props.user} title={`Tickers | KryptoCastle Investments Ltd.`}>
                ...
                <div className="w-full">
                    <DashboardContentCard className={`mb-4`}>
                        <div className="flex flex-row justify-between items-center">
                            ...
                            <button onClick={() => changePassword()} className="btn-no-outline px-4 flex flex-row items-center hover:bg-red-300 transition duration-100 cursor-pointer text-red-500 py-1 border border-red-400 hover:border-red-300 transition duration-750 px-2 bg-red-200 uppercase text-xxs rounded-lg">
                                change password
                            </button>
                        </div>
...
export default AdminSecurityIndexPage;